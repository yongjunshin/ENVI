import os
from src.acc_controller import AdaptiveCruiseControlSystem
...
from src.simulation import non_deterministic_simulation, get_euclidean, get_dtw
...
DISTANCE_FEATURE_IDX = 0
DENORM_CRUISE_CONTROL_DISTANCE_GOAL = 200
NUM_FOLD = 5

for split_tr_FOTs in range(1, 21):
    # user setting
    configs = [[0.5, 0.6, 140], [0.5, 0.9, 140], [0.5, 1.2, 140], [0.5, 1.5, 140], [0.5, 1.8, 140]]
    tr_config_idxs = [0, 2, 4]
    va_config_idxs = [0, 2, 4]
    te_config_idxs = [0, 1, 2, 3, 4]
    split_nums = [split_tr_FOTs, split_tr_FOTs, 10]   #[20, 20, 10]
    current_fold_idx = 4 # less than NUM_FOLD   #todo
    history_len = 10
    common_lr = 0.00005
    common_epoch = 500
    ...
    new_directory = 'D:/ENVI/output/AV/' + current_trial_id
    ...
    np_data, scaler = prepare_np_fot_data(FOT_LOG_DIR, configs_string, COLUMNS, split_nums, NUM_FOLD, current_fold_idx, remove_noise_flag=True)
    ...
    for exp_config in exp_configs:
        ...
        av = AdaptiveCruiseControlSystem(scaler)
        ...
        verif_output_path = "output/AV/" + current_trial_id + "/"
        temp_model_path = "output/AV/temp_models/"
        ...
        fot_minimum_front_distance = get_minimum_front_distance(te_tensor_log, DISTANCE_FEATURE_IDX, DENORM_CRUISE_CONTROL_DISTANCE_GOAL, scaler, history_len)
        fot_maximum_jerks = get_maximum_jerk(te_tensor_log, DISTANCE_FEATURE_IDX, history_len)
        fot_mean_jerks = get_average_jerk(te_tensor_log, DISTANCE_FEATURE_IDX, history_len)

        verification_report = []
        num_nondet_simul_repeat = 5

        if simul_deterministic_flag:
            verification_report.append(configs[:, [1]])
            verification_report.append(fot_minimum_front_distance)
            verification_report.append(fot_maximum_jerks)
            verification_report.append(fot_mean_jerks)
        else:
            verification_report.append(configs[:, [1]].repeat(num_nondet_simul_repeat, 1))
            verification_report.append(fot_minimum_front_distance.repeat(num_nondet_simul_repeat, 1))
        ...
        for idx in range(len(best_models)):
            ...
            simul_minimum_front_distance = get_minimum_front_distance(simul_log, DISTANCE_FEATURE_IDX, DENORM_CRUISE_CONTROL_DISTANCE_GOAL, scaler, history_len)
            simul_maximum_jerks = get_maximum_jerk(simul_log, DISTANCE_FEATURE_IDX, history_len)
            simul_mean_jerks = get_average_jerk(simul_log, DISTANCE_FEATURE_IDX, history_len)

            if simul_deterministic_flag:
                simul_eucls = get_euclidean(simul_log, te_tensor_log, DISTANCE_FEATURE_IDX + 1)
                simul_eucls = torch.reshape(simul_eucls, (simul_eucls.shape[0], 1))
                simul_dtws = get_dtw(simul_log, te_tensor_log, DISTANCE_FEATURE_IDX + 1)
                simul_dtws = torch.reshape(simul_dtws, (simul_dtws.shape[0], 1))
            else:
                simul_eucls = get_euclidean(simul_log, te_tensor_log.repeat(num_nondet_simul_repeat, 1, 1), DISTANCE_FEATURE_IDX + 1)
                simul_eucls = torch.reshape(simul_eucls, (simul_eucls.shape[0], 1))
                simul_dtws = get_dtw(simul_log, te_tensor_log.repeat(num_nondet_simul_repeat, 1, 1), DISTANCE_FEATURE_IDX + 1)
                simul_dtws = torch.reshape(simul_dtws, (simul_dtws.shape[0], 1))

            verification_report.append(simul_minimum_front_distance)
    ...
    simul_traces_for_vis = np.reshape(simul_traces_for_vis,
                                      (simul_traces_for_vis.shape[0], simul_traces_for_vis.shape[1]))
