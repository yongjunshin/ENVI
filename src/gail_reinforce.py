...
class GAILReinforceTrainer:
    ...
    def train(self, model, epochs, tr_dataset, model_dir_path):
        self.optimiser_pi = torch.optim.Adam(model.parameters(), lr=self.lr)

        tr_x = tr_dataset.tr_tensor_x
        tr_y = tr_dataset.tr_tensor_y
        tr_full_configs = tr_dataset.tr_tensor_c
        tr_init_x = tr_dataset.tr_tensor_x_init
        tr_init_configs = tr_dataset.tr_tensor_c_init
        full_logs = tr_dataset.tr_tensor_log

        episode_length = tr_x.shape[1]
        ...
        for epoch in tqdm(range(epochs)):
            ...
            for model_epoch in range(self.ppo_iter):
                ...
                self.train_policy_value_net(log_probs, rewards)
    ...
    def train_policy_value_net(self, log_probs, rewards):
        sim_len = len(log_probs)
        model_loss = 0
        self.optimiser_pi.zero_grad()
        R = torch.zeros(rewards[0].shape, device=self.device)
        for idx in reversed(range(sim_len)):
            R = rewards[idx] + self.gamma * R
            loss = -log_probs[idx] * R
            model_loss = model_loss + loss.mean().item()
            loss.mean().backward()
        self.optimiser_pi.step()
        model_loss = model_loss / sim_len
        print("Model loss:", model_loss, "Model reward:", torch.stack(rewards).mean().item())
